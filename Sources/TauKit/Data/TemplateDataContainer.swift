...
/// `TemplateDataContainer` provides the tangible storage for concrete Swift values, representations of
/// collections, optional value wrappers, and lazy data generators.
internal indirect enum TemplateDataContainer: Equatable, TemplatePrintable {
    // MARK: - Cases
    case bool(Bool)
    case string(String)
    case int(Int)
    ...
    case `nil`(_ type: TemplateDataType)

    /// Lazy resolvable `() -> TemplateData` where return is of `TemplateDataType`
    case lazy(f: () -> TemplateData, returns: TemplateDataType)

    /// A lazy evaluation of the param - Must be generated *only* during `Serialize`(and
    /// subsequently stored in `VariableStack`) to defer evaluation
    case evaluate(param: Parameter.Container)
    ...
    /// The TemplateDataType the container will evaluate to
...
/// Various conveniences for bit ops on template data container
///
/// Note: rawValue of 0 is implicit `Error` type
internal struct State: OptionSet {
    let rawValue: UInt16
