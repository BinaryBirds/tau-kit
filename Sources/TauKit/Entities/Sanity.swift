...
internal extension String {
    func _sanity() {
        precondition(!isKeyword, "Name cannot be keyword")
        precondition(isValidIdentifier, "Name must be valid identifier")
    }
}
...
internal extension Method {
    ...
    static func _sanity() {
        let m = Self.self is MutatingMethod.Type
        let nm = Self.self is NonMutatingMethod.Type
        precondition(m != nm,
                     "Adhere strictly to one and only one of Mutating/NonMutatingMethod")
...
internal extension Array where Element == CallParameter {
    ...
    func confusable(with: Self) -> Bool {
        ...
        var index = 0
        var a: CallParameter { map[index].0 }
        var b: CallParameter? { map[index].1 }
...
internal extension Array where Element == ParseParameter {
    ...
    func splitTuple(_ tuple: Tuple) -> ([String], Tuple)? {
        var parse: [String] = []
        var call: Tuple = .init()

        guard self.count == tuple.count else { return nil }
        var index = 0
        var t: (label: String?, value: Parameter) { tuple.enumerated[index] }
        ...
        while index < self.count {
            switch (s, t.label, t.value.container) {
                ...
                case (.expression(let sE), nil, .expression(let tE))
                    ...
                    let extract: Tuple = .init([tE.first, tE.second, tE.third].compactMap {$0 != nil ? (nil, $0!) : nil})
