/// `Lexer` is an opaque structure that wraps the lexing logic of the template engine.
///
/// Initialized with a `RawTemplate` (raw string-providing representation of a file or other source),
/// used by evaluating with `Lexer.lex()` and either erroring or returning `[Token]`
internal struct Lexer {
    // MARK: - Internal Initializers
    
    /// Init with `RawTemplate`
    init(_ template: RawTemplate) {
        self.src = template
        self.state = .raw
        self.entities = TemplateConfiguration.entities
        self.tagMark = TemplateConfiguration.tagIndicator
        self.lastSourceLocation = (template.state.name, 1, 1)
    }

    // MARK: - Internal
    
    /// Lex the stored `RawTemplate`
    /// - Throws: `TemplateError`
    /// - Returns: An array of fully built `Tokens`, to then be parsed by `Parser`
    ...
    /// Stream of `Tokens` that have been successfully lexed
    private var lexed: [Token] = []
    /// The originating template source content (ie, raw characters)
    private var src: RawTemplate
    ...
    private var escapedTagMark: Token.Container { .raw(tagMark.description) }

    // MARK: - Private - Actual implementation of Lexer

    private mutating func nextToken() throws -> Token.Container? {
        ...
        switch state {
            ...
            case .parameters : var part: Token.Container?
    ...
    private mutating func lexAnonymousTag() -> Token.Container {
        state = .parameters
        depth = 0
        return .tag(nil)
    }

    private mutating func lexNamedTag() throws -> Token.Container {
    ...
    private mutating func lexRaw() -> Token.Container {
        var slice = ""
        scan:
    ...
    private mutating func lexCheckTagIndicator() -> Token.Container {
        pop()
    ...
    private mutating func lexParameters() throws -> Token.Container? {
        ...
        if first.isValidOperator {
            ...
            let twoOp = Operator(rawValue: String([first, current!]))
            let op = twoOp != nil ? twoOp! : Operator(rawValue: String(first))!
        ...
        if let kw = Keyword(rawValue: identifier) { return .param(.keyword(kw)) }
    ...
    /// Convenience for making nested `TemplateError->LexerError.unknownError`
    private func unknownError(_ reason: String) -> TemplateError {
        err(.lexError(.init(.unknownError(reason), src, lexed))) }
    /// Convenience for making nested `TemplateError->LexError.invalidParameterToken`
    private func badToken(_ character: Character) -> TemplateError {
        err(.lexError(.init(.invalidParameterToken(character), src, lexed))) }
    /// Convenience for making nested `TemplateError->LexerError.badOperator`
    private func badOperator(_ op: Operator) -> TemplateError {
        err(.lexError(.init(.invalidOperator(op), src, lexed))) }
    /// Convenience for making nested `TemplateError->LexerError.untermindatedStringLiteral`
    private var unterminatedString: TemplateError {
        err(.lexError(.init(.unterminatedStringLiteral, src, lexed))) }
    
    private mutating func append(_ token: Token.Container) {
