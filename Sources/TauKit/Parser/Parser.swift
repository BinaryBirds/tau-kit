...
internal struct Parser {
    ...
    mutating func parse() throws -> AST {
        literals.stack.append(([], VariableTablePointer.allocate(capacity: 1)))
    ...
    private var openBlocks: [(name: String, block: Function.Type)] = []
    private var lastBlock: Int? { openBlocks.indices.last }
    
    private var offset: Int = 0

    private var peek: Token? { offset < tokens.count ? tokens[offset] : nil }

    private var rawStack: [RawBlock]
    
    private var requiredVars: Set<Variable> = []
    /// Stack of explicitly created variables and whether they've been set yet. Nil special case for define blocks.
    private var createdVars: [[Variable: Bool]] = [[:]]
    
    private var literals: VariableStack
        
    /// Process the next `Token` or multiple tokens.
    private mutating func advance() -> Bool {
        ...
        /// 5meta. Handle unique MetaBlocks
        ...
        return openBlock(tagName, block.0 as! Block, block.1)
    }

    @discardableResult
    private mutating func pop() -> Token? {
    ...
    @discardableResult
    private mutating func append(_ syntax: Parameter) -> Bool {
    ...
    private mutating func appendFunction(_ t: String, _ p: Tuple?) -> Bool {
    ...
    @discardableResult
    private mutating func openBlock(_ n: String, _ b: Block, _ p: Tuple?) -> Bool {
    ...
    private mutating func updateVars(_ vars: Set<Variable>?) -> Bool {
    ...
    private mutating func checkExplicitVariableState(_ v: Variable, _ set: Bool) -> Bool {
        guard createdVars[depth][v] == nil else { return bool(.declared(v.terse)) }
        createdVars[depth][v] = set
        return true
    }
    
    private mutating func handleEvaluateFunction(_ f: String, _ tuple: Tuple, nested: Bool = false) -> Evaluate? {
    ...
    private mutating func handleMeta(_ name: String,
        ...
        switch meta.form {
            case .define:
                ...
                var v: Variable = .atomic(identifier)
            ...
            case .inline:
                ...
                if tuple.count == 2 {
                    ...
                    if case .keyword(.template) = behavior { process = true }
...
//                    rawStack.append(type(of: rawStack.last!).instantiate(size: 0, encoding: Renderer.Option.encoding))
...
//                }
    ...
    private mutating func parseTuple(_ function: String?, retry: Bool = false) -> Tuple? {
        ...
        @discardableResult
        func complexAppend(_ a: Parameter) -> Bool {
            var a = a
            ...
            if case .function(_, .some(let f), let t, _, _) = a.container,
               f as? MetaBlock == nil,
               f as? MutatingMethod == nil,
               f as? UnsafeEntity == nil,
        ...
        func makeVariableIfOpen() -> Bool {
            ...
            if openScope?.isValidIdentifier == false { return bool(.invalidIdentifier(openScope!)) }
            if openMember?.isValidIdentifier == false { return bool(.invalidIdentifier(openMember!)) }
            if let invalid = openPath.first(where: {!$0.isValidIdentifier}) { return bool(.invalidIdentifier(invalid)) }
            let variable = Variable(openScope, openMember, openPath.isEmpty ? nil : openPath)
        ...
        func resolveSubscript(with explicitAccessor: Parameter? = nil) {
            let accessor: Parameter
        ...
        /// Generate a Expression, or if an evaluable, invariant expression, a value
        func express(_ params: [Parameter]) -> Parameter? {
            if let expression = Expression.express(params) {
                return expression.invariant && expression.resolved
                    ? .value(expression.evaluate(&literals))
                    : .expression(expression)
            } else if let expression = Expression.expressTernary(params) {
                return .expression(expression)
            } else if let expression = Expression.expressAny(params) {
                return .expression(expression)
            } else { return nil }
        }
        
        /// Branch on a Operator
        func operatorState(_ op: Operator) {
        ...
        func closeComplex() -> Bool {
            ...
            func countOpsWhere(_ check: (Operator) -> Bool) -> Int {
                exp.reduce(0, { $0 + ($1.operator.map {check($0) ? 1 : 0} ?? 0) }) }
            func firstOpWhere(_ check: (Operator) -> Bool) -> Int? {
                for (i, p) in exp.enumerated() {
                    if let op = p.operator, check(op) { return i } }
                return nil }
            func lastOpWhere(_ check: (Operator) -> Bool) -> Int? {
            ...
            if var ops = opCount as Int?, ops != 0 {
                wrapCalculations:
                for map in Operator.evalPrecedenceMap {
            ...
            if exp[1].operator?.assigning == true {
                ...
                complexes.append([.expression(Expression.express(exp)!)])
                return true
            }
            ...
            exp = [.expression(Expression.expressAny(exp)!)]
            ...
            switch function.0 {
                ...
                case .some(let m) where chained:
                    ...
                    switch result {
                        ...
                        case .success(let M) :
                            let mutating = (try? result.get().first(where: {($0.0 as! Method).mutating}) != nil) ?? false
                            var original: Variable? = nil
...
private extension Array where Element == Dictionary<Variable, Bool> {
    var flat: Set<Variable>? {
        var x: Set<Variable> = []
        reversed().forEach { $0.forEach { if $0.value, !x.contains($0.key) { x.insert($0.key) } } }
        return !x.isEmpty ? x : nil
    }
    
    func match(_ v: Variable) -> (Variable, Bool)? {
