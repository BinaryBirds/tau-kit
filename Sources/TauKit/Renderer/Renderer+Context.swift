public extension Renderer.Context {
    ...
    init?(encodable asSelf: Encodable, isRoot: Bool = false) {
        guard case .dictionary(let d) = asSelf.encodeToTemplateData().container else {
            return nil }
        self.init(d, isRoot: isRoot)
    }
    
    static var defaultContextScope: String { Variable.selfScope }
        
    subscript(scope scope: String = defaultContextScope, key: String) -> TemplateDataRepresentable? {
    ...
    ///
    /// Keys that are valid variable names will be published to (eg, `$app.id`); invalid identifiers will
    ...
    /// globally constant *and* TauKit is already running; literal values may be updated freely prior
    /// to TauKit starting with the caveat that, once declared or locked as literal, it may no longer be
    /// reverted to a variable)
    mutating func setValue(in scope: String = defaultContextScope,
                           at key: String,
                           to value: TemplateDataRepresentable,
    ...
    ///
    /// If scope already exists as a literal but TauKit is running, update will fail. Will initialize context
    /// scope if it does not already exist.
    mutating func setValues(in scope: String = defaultContextScope,
                            to values: [String: TemplateDataRepresentable],
                            allLiteral: Bool = false) throws {
        try literalGuard(allLiteral)
        if scope == Variable.selfScope && allLiteral { throw err("`self` cannot be literal") }
        ...
        if contexts[scopeVar]?.literal ?? false {
            if TemplateConfiguration.isRunning { throw err("\(scope) is a literal scoped context and cannot be updated") }
            assert(allLiteral,
    ...
    mutating func updateValue(in scope: String = defaultContextScope,
                              at key: String,
                              to value: TemplateDataRepresentable) throws {
        ...
        guard isVariable || !TemplateConfiguration.isRunning else {
            throw err("Literal context values cannot be updated after TauKit starts") }
    ...
    /// Cache the current value of `TemplateData` in context for an existing key
    ///
    ...
    ///  * As a context publishing object (must adhere to either `ContextPublisher` [preferred] or
    ///     `TemplateDataRepresentable` resolving to `TemplateData.dictionary`).
    ///  * As a raw object that `UnsafeEntity` objects will have access to during serializing.
    ///
    /// In both cases, `key` represents the access method - for contextual objects, the values it registers
    /// will be published as variables under `$key` scope in the template, and for unsafe objects, tags with access
    ...
    mutating func register(object: Any,
        ...
        if type.contains(.contextual) {
            ...
            if let c = object as? ContextPublisher {
                ...
                values = [:]
                assertionFailure("A registered external object must be either `ContextPublisher` or `TemplateDataRepresentable` vending a dictionary when `type` contains `.contextual`") }
    ...
    mutating func register(generators: [String: TemplateDataGenerator],
    ...
    mutating func clearContext(scope key: String) throws {
        ...
        guard TemplateConfiguration.isRunning else { return }
...
private extension Renderer.Context {
    ...
    func nonExistant(_ scope: String, _ key: String? = nil) -> TemplateError {
        err("\(scope)\(key != nil ? "[\(key!)]" : "") does not exist in context") }
    
    /// Require that the identifier is valid and scope exists
    func validateScope(_ scope: String) throws -> Variable {
        let scopeVar = try getScopeKey(scope)
    ...
    func getScopeKey(_ scope: String) throws -> Variable {
    ...
    func isUpdateable(_ scope: Variable, _ key: String) -> Bool {
        if contexts[scope]?.frozen ?? false { return false }
        if contexts[scope]?.literal ?? false && TemplateConfiguration.isRunning { return false }
        return self[scope, key]?.isVariable ?? true || !TemplateConfiguration.isRunning
    }
