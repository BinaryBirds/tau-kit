...
/// `AST` represents a "compiled," grammatically valid template (which may or may not be fully resolvable or erroring)
public struct AST: Hashable {
    // MARK: - Public Stored Properties
    /// Hashing key for the AST
    public let key: AST.Key
    /// Convenience referrent name for the AST
    public let name: String
    /// A public view of the current state of the AST
    public internal(set) var info: AST.Info

    // MARK: - Internal Stored Properties
    /// The AST scope tables
    var scopes: ContiguousArray<ContiguousArray<Syntax>>
    ...
    /// Whether this AST was obtained from `Cache`
    var cached: Bool
    ...
    /// An opaque object used as a key for storing `AST`s in hash tables (eg, caches). Not directly readable.
    ...
    /// An opaque object passed to a stored `AST` via `Cache`
    ...
    public struct Info {
        ...
        var _requiredVars: Set<Variable> = []
...
internal extension AST {
    ...
    mutating func inline(ast toInline: AST) {
        let stamp = Date()
        var inAST = toInline
    ...
    var summary: String {
        """
        AST `\(name)`: \(scopes.count) tables (\(scopes.count) inlined), \(scopes.count - own.scopes - 1) minimum output bytes
...
private func variablePrecedence(lhs: Variable, rhs: Variable) -> Bool {
