/// Stack of currently defined overlays of `VariableTable`
///
...
internal struct VariableStack {
    ...
    @inline(__always)
    mutating func match(_ variable: Variable) -> TemplateData {
        _match(variable) ?? .error(internal: "No value for \(variable.terse) in context")
    }
    
    /// Locate the `Variable` in the stack, if possible - prefer `match` but useful in some situations to bypass erroring
    @inline(__always)
    mutating func _match(_ variable: Variable) -> TemplateData? {
        func hit(_ v: Variable) -> TemplateData? { context.contexts[.scope(v.scope!)]?.match(v) }
        var err: TemplateData { .error(internal: "No value for \(variable.terse) in context") }
    ...
    func create(_ variable: Variable, _ value: TemplateData?) {
