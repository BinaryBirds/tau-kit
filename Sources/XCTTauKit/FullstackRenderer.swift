...
open class FullstackRendererTestCase: TauKitTestCase {
    open var threads: Int { 1 }
    open var useDefaultSource = true
    
    final public let sources: TemplateSources = .init()
    ...
    private var renderers: [TestRenderer] = []
}
...
private extension FullstackRendererTestCase {
    ...
    func primeRenderers() {
        ...
        started = true
        for _ in (0..<threads) {
            renderers.append(TestRenderer(.init(cache: cache, sources: sources, eventLoop: eLGroup.next())))
        }
    }
}
