...
final class ParserTests: MemoryRendererTestCase {
    ...
    func testCompiler() throws {
        ...
        0: if($:sayhello):
        1: scope(table: 1)
           0: raw(TemplateBuffer: 13B)
           1: for($:names):
           2: scope(table: 2)
              0: raw(TemplateBuffer: 13B)
              1: $:name
              2: raw(TemplateBuffer: 5B)
           3: raw(TemplateBuffer: 9B)
        2: else:
        3: raw(TemplateBuffer: 9B)
        """
    ...
    func testInsertResolution() throws {
        ...
        let preinline = """
        0: inline("header", template):
        1: scope(undefined)
        2: raw(TemplateBuffer: 8B)
        3: evaluate(title):
        4: scope(undefined)
        5: raw(TemplateBuffer: 9B)
        6: inline("header", template):
        7: scope(undefined)
        """

        let expectation = """
        0: inline("header", template):
        1: raw(TemplateBuffer: 12B)
        2: raw(TemplateBuffer: 8B)
        3: evaluate(title):
        4: scope(undefined)
        5: raw(TemplateBuffer: 9B)
        6: inline("header", template):
        7: raw(TemplateBuffer: 12B)
        """
    ...
    func testSerialize() throws {
        ...
        var block = TemplateBuffer.instantiate(size: sampleAST.underestimatedSize, encoding: .utf8)
    ...
    func testEscaping() throws {
        TemplateConfiguration.entities.use(StrToStrMap.escapeHTML, asFunctionAndMethod: "escapeHTML")
    ...
    func testVsComplex() throws {
        ...
        var buffer: ByteBuffer = ByteBufferAllocator().buffer(capacity: 0)
        ...
        for x in 1...10 {
            ...
            if x == 10 { buffer.append(&block) }
        }
        
        print("Average serialize duration: \(timer.average)")
        print("Output size: \(buffer.readableBytes.formatBytes())")
        XCTAssert(true)
    }
    ...
    func testNestedForAndTernary() throws {
        ...
        let serializer = Serializer(sampleAST, [:], TemplateBuffer.self)
        var block = TemplateBuffer.instantiate(size: sampleAST.underestimatedSize, encoding: .utf8)
    ...
    func testAssignmentAndCollections() throws {
        ...
        let serializer = Serializer(parsedAST, ["x": "Hi tdotclare"], TemplateBuffer.self)
        var block = TemplateBuffer.instantiate(size: parsedAST.underestimatedSize, encoding: .utf8)
        let result = serializer.serialize(&block)
        switch result {
    ...
    func testMutatingMethods() throws {
        ...
        let serializer = Serializer(parsedAST, [:], TemplateBuffer.self)
        var block = TemplateBuffer.instantiate(size: parsedAST.underestimatedSize, encoding: .utf8)
        let result = serializer.serialize(&block)
        switch result {
            case .success        : XCTAssertEqual(block.contents, serializeExpected)
    ...
    func testVarStyle() throws {
        ...
        try XCAssertTemplateErrors(render("invalidConstant"), contains: "Can't mutate; `x` is constant")
        ...
        try XCAssertTemplateErrors(render("invalidDeclare"), contains: "Variable `x` used before initialization")
        ...
        try XCAssertTemplateErrors(render("invalidScopeAssign"), contains: "Can't assign; `self.x` is constant")
    }
    ...
    func testSubscriptAssignment() throws {
        ...
        try XCAssertTemplateErrors(render("template"), contains: "Assignment via subscripted access not yet supported")
    }
    ...
    func testIndirectEvalWarning() throws {
        ...
        try XCAssertTemplateErrors(render("A"), contains: "[content()] variable(s) missing")
        try XCAssertTemplateErrors(render("B"), contains: "[content()] variable(s) missing")
        try XCAssertTemplateErrors(render("C"), contains: "[content()] variable(s) missing")
        try XCAssertTemplateErrors(render("D"), contains: "`A` requires parameter semantics for `content()`")
    }
