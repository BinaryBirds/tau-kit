...
final class CachePerformanceTests: FullstackRendererTestCase {
    ...
    func testCacheLinearSpeed() {
        ...
        print(start.distance(to: Date()).formatSeconds())
    ...
    func _linear(_ templates: Int, _ iterations: Int) -> (Double, Int) {
        ...
        (1...templates).forEach { files[String($0)] = "\(offset).0" }
...
final class NIOFilesTests: FullstackRendererTestCase {
    ...
    lazy var nioFiles = FileSource(fileio: fileio,
                                     limits: .default,
                                     sandboxDirectory: templateFolder,
                                     viewDirectory: templateFolder + "SubTemplates/")
    
    override var source: Source { get { nioFiles } set {} }
    ...
    func testNIOFileSandbox() throws {
        TemplateConfiguration.entities.use(IntFormatterMap.bytes, asMethod: "formatBytes")
        ...
        renderBuffer("../test").whenComplete {
            try? XCTAssertNoThrow($0.get()) }
        renderBuffer("../../test").whenComplete {
            try? XCAssertTemplateErrors($0.get(), contains: "Attempted to escape sandbox") }
        renderBuffer(".test").whenComplete {
            try? XCAssertTemplateErrors($0.get(), contains: "Attempted to access hidden file `.test`") }
    }
}
...
final class MultisourceTests: FullstackRendererTestCase {
    ...
    var one: MemorySource { source as! MemorySource }
    let two: MemorySource = .init()
    let hidden: MemorySource = .init()
    ...
    func testEmptySources() throws {
        ...
        renderBuffer("a").whenComplete {
            try? XCAssertTemplateErrors($0.get(), contains: "No searchable sources exist") }
    }
    ...
    func testMultipleSources() throws {
        ...
        renderBuffer("c").whenComplete {
            try? XCAssertTemplateErrors($0.get(), contains: "No template found") }
